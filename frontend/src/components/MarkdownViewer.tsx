...
export function MarkdownViewer({ content }: MarkdownViewerProps) {
  ...
  const flushList = (key: number) => {
    ...
    const listEl = (
      <ul
        key={`list-${key}`}
        className="my-3 space-y-1.5 pl-4 text-xs font-mono text-[var(--text-main)]"
      >
    ...
    while (remaining.length > 0) {
      ...
      if (firstMatch.type === 'bold') {
        ...
        parts.push(
          <code
            key={`c-${keyCounter++}`}
            className="rounded bg-[rgba(0,240,255,0.15)] px-1.5 py-0.5 font-mono text-[0.78rem] text-[var(--accent-cyan)] border border-[rgba(0,240,255,0.3)]"
          >
            {firstMatch.match[1]}
          </code>,
        );
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed === '---' || trimmed === '***' || trimmed === '___') {
      elements.push(
        <hr
          key={`hr-${i}`}
          className="my-5 border-0 h-[1px] bg-gradient-to-r from-transparent via-[rgba(0,240,255,0.4)] to-transparent"
        />,
      );
      continue;
    }
    ...
    if (trimmed.startsWith('# ')) {
      elements.push(
        <h1
          key={`h1-${i}`}
          className="mb-4 mt-2 font-display text-xl font-black tracking-wider text-[#ffffff] [text-shadow:0_0_12px_var(--accent-cyan)]"
        >
    ...
    if (trimmed.startsWith('## ')) {
      elements.push(
        <h2
          key={`h2-${i}`}
          className="mb-3 mt-6 font-display text-base font-black tracking-wide text-[var(--accent-pink)] [text-shadow:0_0_8px_rgba(255,0,127,0.5)]"
        >
    ...
    if (trimmed.startsWith('### ')) {
      elements.push(
        <h3
          key={`h3-${i}`}
          className="mb-2 mt-4 font-display text-sm font-bold text-[var(--accent-cyan)]"
        >
