		...
		const handleEngineEvent = (state: unknown) => {
			...
			if (type === 'piece_moved') {
				...
				const runStepAnimation = () => {
					...
					if (e.clashOutcome === 'attacker_won') {
						setMoveLogs((prev) => [
							{ ck: e.color, text: t('game.clashAttackerWon', { to: e.to }) },
							...prev.slice(0, 11),
						])
					} else if (e.clashOutcome === 'defender_won') {
						setMoveLogs((prev) => [
							{ ck: e.color, text: t('game.clashDefenderWon') },
							...prev.slice(0, 11),
						])
					} else if (e.captured) {
						setMoveLogs((prev) => [
