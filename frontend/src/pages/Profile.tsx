...
export function Profile() {
  ...
  if (loading) {
    return <div style={{ color: '#b8a9d4', textAlign: 'center', marginTop: 80, fontSize: 18 }}>{t('profile.loadingProfile')}</div>
  }

  if (!profile) {
    return (
      <div style={{ color: '#ff6b8a', textAlign: 'center', marginTop: 80, fontSize: 18, fontWeight: 600 }}>
        ...
        <div style={{ ...card, padding: 36, display: 'flex', alignItems: 'center', gap: 32, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative' }}>
            <UserAvatar 
              ...
              style={{ boxShadow: '0 0 0 4px rgba(167,139,250,0.35), 0 0 24px rgba(244,114,182,0.4)' }}
            ...
            <span
              ...
              style={{
                ...
                background: statusStyle.color, border: '3px solid #0f0a1a', boxShadow: `0 0 8px ${statusStyle.color}`,
              }}
            />
            {isOwnProfile && (
              <div 
                ...
                style={{
                  ...
                  background: 'rgba(20,14,35,0.75)', display: 'flex', alignItems: 'center', justifyContent: 'center',
                  ...
                  boxShadow: '0 0 0 4px rgba(167,139,250,0.35), 0 0 24px rgba(244,114,182,0.4)',
                }}
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ color: '#b8a9d4', fontSize: 14, marginTop: 4, fontWeight: 500 }}>
              {t('profile.memberSince')} {new Date(profile.createdAt).toLocaleDateString()}
            </div>
            {isOwnProfile && (
              <div style={{ display: 'flex', gap: 10, marginTop: 12, alignItems: 'center' }}>
                <span onClick={handleRemoveAvatar} style={{ color: '#ff6b8a', fontSize: 12.5, fontWeight: 700, cursor: 'pointer', opacity: 0.85 }} onMouseOver={(e) => e.currentTarget.style.opacity = '1'} onMouseOut={(e) => e.currentTarget.style.opacity = '0.85'}>
                  Remove Avatar
                </span>
                {uploadError && <span style={{ color: '#ff6b8a', fontSize: 12.5, fontWeight: 700 }}>· {uploadError}</span>}
              </div>
            )}
          </div>

          <div style={{ textAlign: 'center', background: 'linear-gradient(145deg, rgba(40,28,65,0.8), rgba(25,18,42,0.9))', padding: '18px 30px', borderRadius: 20, border: '1px solid rgba(167,139,250,0.2)', boxShadow: '0 8px 20px -6px rgba(0,0,0,0.5)' }}>
            <div style={{ fontSize: 12, color: '#a78bfa', textTransform: 'uppercase', letterSpacing: 1.5, fontWeight: 800, fontFamily: "'Space Grotesk', 'Outfit', sans-serif" }}>{t('dashboard.rating')}</div>
            <div style={{ fontSize: 34, fontWeight: 900, color: '#ffd66b', marginTop: 4, fontFamily: "'Space Grotesk', 'Outfit', sans-serif", textShadow: '0 0 14px rgba(255,214,107,0.3)' }}>♛ {profile.rating}</div>
          </div>
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 18 }}>
          <StatBox label={t('profile.wins')} value={profile.wins} color="#a78bfa" />
          <StatBox label={t('profile.losses')} value={profile.losses} color="#ff6b8a" />
          <StatBox label={t('profile.winRate')} value={`${winRate}%`} color="#6bb8ff" />
          <StatBox label={t('profile.bestStreak')} value={profile.bestWinStreak} color="#ffd66b" />
        </div>

        <div style={{ ...card, padding: 30 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 22 }}>
            <h3 style={{ margin: 0, fontSize: 20, color: '#f8f0ff', fontWeight: 900, fontFamily: "'Space Grotesk', 'Outfit', sans-serif" }}>{t('profile.recentMatches')}</h3>
            {gamesData && gamesData.total > 0 && (
              <div style={{ color: '#b8a9d4', fontSize: 14, fontWeight: 600 }}>{t('profile.gamesPlayedCount', { count: gamesData.total })}</div>
            )}
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {!gamesData || gamesData.games.length === 0 ? (
              <div style={{ color: '#b8a9d4', fontStyle: 'italic', padding: '24px 0', textAlign: 'center', background: 'rgba(255,255,255,0.02)', borderRadius: 16, border: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              gamesData.games.map((game) => {
                ...
                const resultColor = isWinner ? '#a78bfa' : (isDraw ? '#b8a9d4' : '#ff6b8a')
                ...
                return (
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
                      ...
                      <div style={{ color: '#f8f0ff', fontWeight: 700, fontSize: 15, fontFamily: "'Space Grotesk', 'Outfit', sans-serif" }}>
                        {t('profile.vsLabel')} {opponents.length > 0 ? opponents.map(o => o.username).join(', ') : t('profile.botsUnknown')}
                      </div>
                    </div>

                    <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
                      <div style={{ color: '#ffd66b', fontSize: 13.5, fontWeight: 700, fontFamily: "'Space Grotesk', 'Outfit', sans-serif" }}>
                        {t('profile.goalsCount', { count: game.piecesInGoal })}
                      </div>
                      <div style={{ color: '#b8a9d4', fontSize: 13, minWidth: 80, textAlign: 'right' }}>
...
function StatBox({ label, value, color }: { label: string; value: React.ReactNode; color: string }) {
  return (
    <div style={{ ...card, padding: '24px 20px', display: 'flex', flexDirection: 'column', gap: 6, position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ color: '#b8a9d4', fontSize: 13, fontWeight: 800, textTransform: 'uppercase', letterSpacing: 1.5, fontFamily: "'Space Grotesk', 'Outfit', sans-serif" }}>
        {label}
      </div>
  ...
  return (
    <div style={{ ...card, padding: 24, display: 'flex', flexDirection: 'column', gap: 16, position: 'sticky', top: 32 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <h3 style={{ margin: 0, fontSize: 19, color: '#f8f0ff', fontWeight: 900, fontFamily: "'Space Grotesk', 'Outfit', sans-serif" }}>
          {t('nav.friends')}
        </h3>
        <div style={{ background: 'linear-gradient(135deg, #a78bfa, #6bb8ff)', color: '#0f0a1a', padding: '2px 9px', borderRadius: 12, fontSize: 12, fontWeight: 800, fontFamily: "'Space Grotesk', 'Outfit', sans-serif" }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {!friends ? (
          <div style={{ color: '#b8a9d4', fontSize: 13, textAlign: 'center', padding: '20px 0' }}>{t('common.loading')}</div>
        ) : friends.length === 0 ? (
          <div style={{ color: '#b8a9d4', fontSize: 13, textAlign: 'center', padding: '20px 0', background: 'rgba(255,255,255,0.02)', borderRadius: 14, border: '1px solid rgba(255,255,255,0.06)' }}>
            {t('profile.noFriendsShort')}<br/><br/>
            <span style={{ color: '#a78bfa', cursor: 'pointer', fontWeight: 700 }}>{t('profile.findPlayers')}</span>
          </div>
        ) : (
          friends.map((friend) => {
              ...
              >
                <div style={{ position: 'relative', marginRight: 12, flexShrink: 0 }}>
                  <UserAvatar 
                    ...
                    style={{ boxShadow: '0 0 0 2px rgba(167,139,250,0.3)' }}
                  />
                  <span
                    style={{
                      position: 'absolute', right: -1, bottom: -1, width: 11, height: 11, borderRadius: '50%',
                      background: status.color, border: '2px solid #0f0a1a', boxShadow: `0 0 6px ${status.color}`,
                    }}
                  />
                </div>

                <div style={{ flex: 1, overflow: 'hidden' }}>
                  <div style={{ fontWeight: 700, fontSize: 14.5, color: '#f8f0ff', whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden', fontFamily: "'Space Grotesk', 'Outfit', sans-serif" }}>
                    {friend.username}
                  </div>
                ...
                {/* Rating Badge */}
                <div style={{
                  padding: '4px 10px', borderRadius: 8, border: '1px solid rgba(255,214,107,0.4)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  background: 'rgba(255,214,107,0.12)',
                  color: '#ffd66b', fontSize: 12, fontWeight: 800, flexShrink: 0, fontFamily: "'Space Grotesk', 'Outfit', sans-serif"
                }}>
                  ♛ {friend.rating}
                </div>
              </div>
