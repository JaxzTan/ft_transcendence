...
export class LudoEngine {
  ...
  // (or is cancelled when a player reaches the target early).
  ...
  private armClashPhaseTimers(gameId: string, onPressStart?: () => void): void {
    ...
    void this.store.loadClashState(gameId).then((clash) => {
      ...
      // and wrapping would deadlock the same promise chain (observed live: bots pressed,
      // deadline passed, clash never resolved).
  ...
  async movePiece(gameId: string, pieceId: PieceId): Promise<MovePieceOutput> {
    ...
    // roll and move is rejected here.
