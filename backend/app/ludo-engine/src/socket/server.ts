...
export class SocketServer {
  ...
  constructor() {
    ...
    this.handlers = new SocketHandlers(
      this.store, this.engine, this.clashManager, this.queue,
      this.userIdMap, (g, c) => this.getOrCreateBot(g, c),
    );
    ...
    this.engine.onEvent((event) => {
      ...
      } else if (event.type === 'dice_rolled' || event.type === 'piece_moved' || event.type === 'game_started') {
  ...
  private triggerBotTurn(gameId: string): void {
    this.queue.enqueue(gameId, async () => {
      ...
      if (!this.isBotPlayer(gameId, state.currentTurn)) return;

      const bot = this.getOrCreateBot(gameId, state.currentTurn);
      await bot.takeTurn();
